...
public abstract class BasePage<T> extends GenericWebPage<T> {
	...
	private void commonInit(PageParameters pageParameters) {
        pageTitel = new Label("pageTitle", getText("global.page.title",
                "global.page.title"));
        add(pageTitel);
	...
	protected Navbar newNavbar(String markupId) {
		...
		// show brand name
        navbar.brandName(Model.of(getText("global.page.title").getString()));

		navbar.addComponents(NavbarComponents.transform(
                Navbar.ComponentPosition.LEFT, new NavbarButton<Homepage>(
                        Homepage.class, Model.of(getText("nav.home").getString()))
		...
		navbar.addComponents(NavbarComponents.transform(
				Navbar.ComponentPosition.LEFT, new NavbarButton<GraphPage>(
                        GraphPage.class, Model.of(getText("nav.graph").getString()))
                        .setIconType(IconType.picture)));

		navbar.addComponents(NavbarComponents.transform(
				Navbar.ComponentPosition.LEFT, new NavbarButton<GraphPage>(
                        QueryPage.class, Model.of(getText("nav.query").getString()))
                        .setIconType(IconType.filter)));

		navbar.addComponents(NavbarComponents.transform(
				Navbar.ComponentPosition.LEFT,
                new NavbarButton<DegreePage>(DegreePage.class, Model.of(getText(
		...
		navbar.addComponents(NavbarComponents.transform(
				Navbar.ComponentPosition.RIGHT, new NavbarButton<UserPage>(
                        UserPage.class, Model.of(getText("nav.users").getString()))
	...
	private Component newCompanyDropDownButton() {
        Component btn = new NavbarDropDownButton(Model.of(getText("nav.companies")
                .getString())) {
